...
class EnseignantController extends Controller
{
    ...
    public function store(Request $request) { $this->normaliseRelationIds($request); $validated=$this->validateEnseignant($request); $user=auth()->user(); $teacher=DB::transaction(function()use($validated,$user,$request){$attrs=$this->attributes($validated,$user,$request);$teacher=Enseignant::create($attrs);// Créer l'utilisateur correspondant dans la table users pour permettre la connexion$appUser=User::create(['tenant_id'=>$attrs['tenant_id'],'etablissement_id'=>$attrs['etablissement_id'],'nom'=>$validated['nom'],'prenom'=>$validated['prenoms'],'email'=>$validated['email'],'telephone'=>$validated['telephone'],'password'=>Hash::make('12345678'),'role'=>'enseignant','statut'=>'actif','client_id'=>$user->client_id??$user->id,]);$teacher->update(['user_id'=>$appUser->id]);$teacher->matieres()->sync($validated['matiere_ids']);$teacher->classes()->sync($validated['classe_ids']);$teacher->series()->sync($validated['serie_ids']??[]);return $teacher->load(['matieres','classes','series']);}); return $this->response($request,$teacher,'Enseignant créé avec succès.'); }
    public function update(Request $request, Enseignant $enseignant) { $this->authorizeTenant($enseignant);$this->normaliseRelationIds($request);$validated=$this->validateEnseignant($request,$enseignant);$teacher=DB::transaction(function()use($validated,$enseignant,$request){$enseignant->update($this->attributes($validated,null,$request,$enseignant));// Synchroniser aussi l'utilisateur liéif($enseignant->user_id){User::where('id',$enseignant->user_id)->update(['nom'=>$validated['nom'],'prenom'=>$validated['prenoms'],'email'=>$validated['email'],'telephone'=>$validated['telephone'],]);}$enseignant->matieres()->sync($validated['matiere_ids']);$enseignant->classes()->sync($validated['classe_ids']);$enseignant->series()->sync($validated['serie_ids']??[]);return $enseignant->fresh(['matieres','classes','series']);});return $this->response($request,$teacher,'Enseignant mis à jour avec succès.'); }
    public function destroy(Enseignant $enseignant) { $this->authorizeTenant($enseignant);$enseignant->matieres()->detach();$enseignant->classes()->detach();$enseignant->series()->detach();$appUserId=$enseignant->user_id;$enseignant->delete();// Supprimer aussi l'utilisateur liéif($appUserId){User::where('id',$appUserId)->delete();}return request()->expectsJson()?response()->json(['success'=>true,'message'=>'Enseignant supprimé avec succès.']):back()->with('success','Enseignant supprimé avec succès.'); }
    ...
    private function validateEnseignant(Request $request, ?Enseignant $teacher=null): array { $user=auth()->user();$ignoreId=$teacher?->id;return $request->validate(['nom'=>['required','string','max:255'],'prenoms'=>['required','string','max:255'],'email'=>['required','email','max:255',Rule::unique('enseignants','email')->ignore($ignoreId),Rule::unique('users','email')->ignore($teacher?->user_id)],'telephone'=>['required','string','max:50'],'matricule'=>['required','string','max:100',Rule::unique('enseignants','matricule')->ignore($ignoreId)],'nombre_annees_enseignement'=>['required','integer','min:0','max:80'],'sexe'=>['required','in:Masculin,Féminin'],'photo'=>['nullable','image','mimes:jpg,jpeg,png,webp','max:2048'],'matiere_ids'=>['required','array','min:1','max:2'],'matiere_ids.*'=>[Rule::exists('matieres','id')->where(fn($q)=>$q->where('tenant_id',$user->tenant_id))],'classe_ids'=>['required','array','min:1'],'classe_ids.*'=>[Rule::exists('classes','id')->where(fn($q)=>$q->where('tenant_id',$user->tenant_id))],'serie_ids'=>['nullable','array'],'serie_ids.*'=>[Rule::exists('series','id')->where(fn($q)=>$q->where('tenant_id',$user->tenant_id))]]); }
