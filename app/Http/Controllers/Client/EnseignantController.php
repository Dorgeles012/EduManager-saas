...
class EnseignantController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $teacher = DB::transaction(function () use ($validated, $user, $request) {
            ...
            // Créer l'utilisateur correspondant dans la table users pour permettre la connexion
            $appUser = User::create([
        ...
        return $this->response($request, $teacher, 'Enseignant créé avec succès.');
    }
    ...
    public function update(Request $request, Enseignant $enseignant)
    {
        ...
        $teacher = DB::transaction(function () use ($validated, $enseignant, $request) {
            $enseignant->update($this->attributes($validated, null, $request, $enseignant));

            // Synchroniser aussi l'utilisateur lié
            ...
            return $enseignant->fresh(['matieres', 'classes', 'series']);
        });

        return $this->response($request, $teacher, 'Enseignant mis à jour avec succès.');
    }
    ...
    public function destroy(Enseignant $enseignant)
    {
        ...
        return request()->expectsJson()
            ? response()->json(['success' => true, 'message' => 'Enseignant supprimé avec succès.'])
            : back()->with('success', 'Enseignant supprimé avec succès.');
    }
    ...
    private function validateEnseignant(Request $request, ?Enseignant $teacher = null): array
    {
        ...
        return $request->validate([
            ...
            'email' => ['required', 'email', 'max:255', Rule::unique('enseignants', 'email')->ignore($ignoreId), Rule::unique('users', 'email')->ignore($teacher?->user_id)],
            'telephone' => ['required', 'string', 'max:50'],
            'matricule' => ['required', 'string', 'max:100', Rule::unique('enseignants', 'matricule')->ignore($ignoreId)],
    ...
    private function response(Request $request, Enseignant $teacher, string $message)
    {
        return $request->expectsJson()
            ? response()->json(['success' => true, 'message' => $message, 'teacher' => $this->teacherPayload($teacher)])
