...
class EmploiTempsController extends Controller
{
    ...
    public function destroyTeacherSchedule(Request $request, Enseignant $enseignant) { $this->guardTeacher($enseignant,$request); $deleted=EmploiTemps::where('tenant_id',$request->user()->tenant_id)->where('enseignant_id',$enseignant->id)->delete(); if(!$deleted) return response()->json(['message'=>'Aucun emploi du temps à supprimer.'],404); return response()->json(['message'=>'Emploi du temps supprimé avec succès.']); }
