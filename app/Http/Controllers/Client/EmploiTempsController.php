...
class EmploiTempsController extends Controller
{
    ...
    /**
     * Créneaux par défaut utilisés uniquement lorsqu'aucun créneau
     * n'est encore enregistré en base pour un enseignant.
     */
    ...
    public function index(Request $request)
    {
        ...
        return view('client.emploi-temps', [
            'entries' => EmploiTemps::with(['matiere', 'enseignant', 'classe'])
            ...
            'years' => AnneeAcademique::where('tenant_id', $user->tenant_id)
                ...
                ->get(),
            'subjects' => Matiere::where('tenant_id', $user->tenant_id)
                ->orderBy('nom')
                ->get(),
            'establishments' => Etablissement::where('tenant_id', $user->tenant_id)
                ->get()
        ]);
    }
    ...
    {
        $this->guardTeacher($enseignant, $request);
        return view('client.emploi-temps-show', $this->scheduleData($request, $enseignant));
    }
    ...
    public function storeTeacherSchedule(Request $request, Enseignant $enseignant)
    {
        ...
        return response()->json([
            ...
            'redirect' => route('client.emploi-temps.show', $enseignant)
        ]);
    }
    ...
    {
        $this->guardTeacher($enseignant, $request);
        return view('client.emploi-temps-print', $this->scheduleData($request, $enseignant));
    }

    public function pdfTeacher(Request $request, Enseignant $enseignant)
    {
        $this->guardTeacher($enseignant, $request);
        return Pdf::loadView('client.emploi-temps-print', $this->scheduleData($request, $enseignant))
            ->setPaper('a4', 'landscape')
            ->download('emploi-du-temps-' . $enseignant->id . '.pdf');
    }

    // Historical class timetable endpoints remain available.
    public function teachers(Request $request, Classe $classe)
    {
        abort_unless((int)$classe->tenant_id === (int)$request->user()->tenant_id, 404);
        return response()->json(
            $classe->enseignants()
                ->orderBy('nom')
                ->get(['enseignants.id', 'nom', 'prenoms'])
        );
    }
    ...
    public function store(Request $request)
    {
        ...
        $data = $this->normaliseEntry($data, $request);
        $this->validateExternalConflicts($request, $data, null);
        EmploiTemps::create($data + ['tenant_id' => $request->user()->tenant_id]);
        return back()->with('success', 'Cours ajouté à l\'emploi du temps.');
    }

    public function destroy(Request $request, EmploiTemps $emploiTemps)
    {
        abort_unless((int)$emploiTemps->tenant_id === (int)$request->user()->tenant_id, 404);
        $emploiTemps->delete();
    ...
    {
        return view('client.emploi-temps-class-print', $this->classScheduleData($request));
    }

    public function pdf(Request $request)
    {
        return Pdf::loadView('client.emploi-temps-class-print', $this->classScheduleData($request))
            ->setPaper('a4', 'landscape')
            ->download('emploi-du-temps.pdf');
    }

    private function form(Request $request, Enseignant $enseignant, bool $editing)
    {
        return view('client.emploi-temps-create', $this->scheduleData($request, $enseignant) + ['editing' => $editing]);
    }

    private function scheduleData(Request $request, Enseignant $enseignant): array
    {
        $user = $request->user();
        $enseignant->loadMissing(['matiere', 'classes']);
    ...
    private function saveSchedule(Request $request, Enseignant $enseignant): void
    {
        $user = $request->user();
        $data = $request->validate([
            'annee_academique_id' => ['nullable', 'integer', 'exists:annee_academique,id'],
            ...
            'cells.*.classe_id' => ['nullable', 'integer', 'exists:classes,id'],
            'cells.*.serie_id' => ['nullable', 'integer', 'exists:series,id'],
            'cells.*.matiere_id' => ['nullable', 'integer', 'exists:matieres,id'],
        ...
        if (!empty($data['annee_academique_id']) && !AnneeAcademique::where('tenant_id', $user->tenant_id)
            ->where('id', $data['annee_academique_id'])
            ->when($establishmentId, fn ($q) => $q->where('etablissement_id', $establishmentId))
            ->exists()) {
        ...
        foreach ($data['cells'] ?? [] as $key => $cell) {
            if (empty($cell['classe_id'])) continue;
            $parts = explode('|', $key);
            if (count($parts) !== 2 || !in_array($parts[0], self::DAYS, true) || !isset($slotDefinitions[$parts[1]])) {
                throw ValidationException::withMessages(['cells' => 'Créneau invalide.']);
            }
            
            $subject = (int)($cell['matiere_id'] ?? 0);
            $serie = empty($cell['serie_id']) ? null : (int)$cell['serie_id'];
            
            if (!in_array($subject, $allowedSubjects, true)) {
                throw ValidationException::withMessages(['cells' => 'La matière choisie n\'est pas attribuée à cet enseignant.']);
            }
            if ($serie && !in_array($serie, $allowedSeries, true)) {
                throw ValidationException::withMessages(['cells' => 'La série choisie n\'est pas attribuée à cet enseignant.']);
            }
            
            if (!Classe::where('tenant_id', $user->tenant_id)
                ->where('id', (int)$cell['classe_id'])
                ->where('etablissement_id', $establishmentId)
                ->exists()) {
                throw ValidationException::withMessages(['cells' => 'Classe non autorisée.']);
            }
            
            $slot = $slotDefinitions[$parts[1]];
            $entries[] = $this->normaliseEntry([
                'jour' => $parts[0],
                'heure_debut' => $slot['heure_debut'],
                'heure_fin' => $slot['heure_fin'],
                'slot_key' => $parts[1],
                'classe_id' => (int)$cell['classe_id'],
                'serie_id' => $serie,
                'matiere_id' => $subject,
        ...
        DB::transaction(function() use ($request, $enseignant, $entries, $slotDefinitions) {
            EmploiTemps::where('tenant_id', $request->user()->tenant_id)
                ->where('enseignant_id', $enseignant->id)
                ->delete();
            EmploiTempsSlot::where('tenant_id', $request->user()->tenant_id)
                ->where('enseignant_id', $enseignant->id)
    ...
    private function validateInternalConflicts(array $entries): void
    {
        foreach ($entries as $i => $a) {
            foreach ($entries as $j => $b) {
                ...
                $sameSeries = $a['serie_id'] === $b['serie_id'];
                if ($a['classe_id'] === $b['classe_id'] && $sameSeries) {
                    throw ValidationException::withMessages(['cells' => 'Cette classe et cette série sont déjà occupées sur ce créneau.']);
                }
                if ($a['enseignant_id'] === $b['enseignant_id']) {
                    throw ValidationException::withMessages(['cells' => 'Cet enseignant enseigne déjà sur ce créneau.']);
                }
                if ($a['serie_id'] && $sameSeries) {
                    throw ValidationException::withMessages(['cells' => 'Cette série est déjà occupée sur ce créneau.']);
                }
    ...
    private function validateExternalConflicts(Request $request, array $entry, ?int $replacedTeacherId): void
    {
        ...
        foreach (['etablissement_id', 'annee_academique_id'] as $column) {
            $entry[$column] === null 
                ? $base->whereNull('emploi_temps.' . $column) 
                : $base->where('emploi_temps.' . $column, $entry[$column]);
        }
        ...
        $entry['serie_id'] === null 
            ? $classConflict->whereNull('emploi_temps.serie_id') 
            : $classConflict->where('emploi_temps.serie_id', $entry['serie_id']);
        ...
        if ($entry['serie_id'] && (clone $base)->where('emploi_temps.serie_id', $entry['serie_id'])->exists()) {
            throw ValidationException::withMessages(['cells' => 'Cette série est déjà occupée sur ce créneau.']);
        }
    ...
    {
        $user = $request->user();
        $class = Classe::where('tenant_id', $user->tenant_id)
            ->when($user->etablissement_id, fn($q) => $q->where('etablissement_id', $user->etablissement_id))
            ->findOrFail($request->integer('classe_id'));
        
        $yearId = $request->integer('annee_academique_id');
        $entries = EmploiTemps::with(['matiere', 'enseignant', 'serie'])
            ->where('tenant_id', $user->tenant_id)
        ...
        return [
            ...
            'days' => self::DAYS,
            'slots' => $slots,
            'school' => Etablissement::find($class->etablissement_id),
            'year' => $yearId ? AnneeAcademique::where('tenant_id', $user->tenant_id)->find($yearId) : null,
            'totalSeances' => $entries->count()
        ];
    }
    ...
    private function validatedSlots(array $submittedSlots): array
    {
        ...
        foreach (self::DEFAULT_SLOTS as $slot) {
            ...
            if (!is_array($value)) {
                throw ValidationException::withMessages(['slots' => 'Un créneau horaire est manquant.']);
            }
            if (!$this->timeAt($value['heure_debut'])->lt($this->timeAt($value['heure_fin']))) {
                throw ValidationException::withMessages(['slots' => 'L\'heure de fin doit être supérieure à l\'heure de début.']);
            }
            $definitions[$slot['key']] = [
                'heure_debut' => $value['heure_debut'],
                'heure_fin' => $value['heure_fin']
            ];
        }

        // Vérifier les doublons (créneaux avec mêmes horaires)
        $seen = [];
        foreach ($definitions as $key => $slot) {
            $signature = $slot['heure_debut'] . '-' . $slot['heure_fin'];
            if (isset($seen[$signature])) {
                throw ValidationException::withMessages(['slots' => 'Deux créneaux identiques ne sont pas autorisés dans le même emploi du temps.']);
            }
            $seen[$signature] = $key;
        }

        // Trier chronologiquement par heure_debut
        uasort($definitions, fn($a, $b) => 
            $this->timeAt($a['heure_debut'])->timestamp <=> $this->timeAt($b['heure_debut'])->timestamp
        );
    ...
    private function normaliseEntry(array $entry, Request $request): array
    {
        $entry['heure_debut'] = $this->timeAt($entry['heure_debut'])->format('H:i:s');
        $entry['heure_fin'] = $this->timeAt($entry['heure_fin'])->format('H:i:s');
        $entry['serie_id'] = $entry['serie_id'] ?? null;
        $entry['salle'] = filled($entry['salle'] ?? null) ? trim($entry['salle']) : null;
        $entry['etablissement_id'] = $entry['etablissement_id'] ?? $request->user()->etablissement_id;
        $entry['annee_academique_id'] = $entry['annee_academique_id'] ?? null;
        return $entry;
    }
