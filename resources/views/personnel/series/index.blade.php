...
<div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
    <div>
        <h2 class="font-headline-lg text-headline-lg text-on-surface">
            <span class="text-primary">Gestion des Séries</span>
        </h2>
                ...
                <tr class="hover:bg-primary/5 transition-colors group">
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex justify-end gap-2">
                            <a href="{{ route('personnel.series.disciplines', $s['id']) }}" class="px-3 py-2 text-primary hover:bg-primary/10 rounded-lg transition-all flex items-center gap-1" title="Gérer les disciplines">
                            ...
                            <form action="{{ route('personnel.series.destroy', $s['id']) }}" method="POST" class="inline delete-series-form">
                                @csrf
                                @method('DELETE')
...
<div class="hidden fixed inset-0 z-[60] flex items-center justify-center p-4" id="addModal">
    ...
    <div class="bg-white w-full max-w-md rounded-xl shadow-2xl relative z-10 overflow-hidden transform transition-all scale-95 opacity-0 duration-300" id="addModalContent">
        ...
        <form class="p-6 space-y-5" id="addSeriesForm" action="{{ route('personnel.series.store') }}" method="POST">
            @csrf
            <div class="space-y-2">
...
<script>
    ...
    function editSeries(id, nomSerie, classeIds) {
        document.getElementById('editSeriesForm').action = `{{ url('/personnel/series') }}/${id}`;
