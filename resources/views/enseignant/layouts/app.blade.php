...
<head>
    ...
    <title>@yield('title', 'EduManager - Tableau de bord Enseignant')</title>
...
<body class="font-body-md text-body-md overflow-x-hidden">
    ...
    <aside class="fixed left-0 top-0 h-screen w-[260px] bg-surface-container-lowest flex flex-col border-r border-outline-variant z-50">
        ...
        <nav class="sidebar-nav mt-2">
            ...
            <a class="flex items-center gap-3 px-6 py-3 text-on-surface-variant hover:bg-surface-container transition-colors {{ $currentRoute === 'enseignant.dashboard' ? 'nav-active' : '' }}" href="{{ route('enseignant.dashboard') }}">
                <span class="material-symbols-outlined">dashboard</span>
                <span class="font-label-md text-label-md">Dashboard</span>
            </a>

            <a class="flex items-center gap-3 px-6 py-3 nav-disabled" href="#" title="Accessible uniquement en consultation." onclick="return false;">
                <span class="material-symbols-outlined">calendar_today</span>
                <span class="font-label-md text-label-md">Année académique</span>
            </a>

            <a class="flex items-center gap-3 px-6 py-3 text-on-surface-variant hover:bg-surface-container transition-colors {{ str_starts_with($currentRoute, 'enseignant.notes') ? 'nav-active' : '' }}" href="{{ route('enseignant.notes.index') }}">
                <span class="material-symbols-outlined">grade</span>
                <span class="font-label-md text-label-md">Notes</span>
            </a>

            <a class="flex items-center gap-3 px-6 py-3 text-on-surface-variant hover:bg-surface-container transition-colors {{ str_starts_with($currentRoute, 'enseignant.bulletin') ? 'nav-active' : '' }}" href="{{ route('enseignant.bulletin.index') }}">
                <span class="material-symbols-outlined">description</span>
                <span class="font-label-md text-label-md">Bulletin</span>
            </a>

            <a class="flex items-center gap-3 px-6 py-3 text-on-surface-variant hover:bg-surface-container transition-colors {{ str_starts_with($currentRoute, 'enseignant.emploi-temps') ? 'nav-active' : '' }}" href="{{ route('enseignant.emploi-temps.index') }}">
            ...
            <a class="flex items-center gap-3 px-6 py-3 text-on-surface-variant hover:bg-surface-container transition-colors" href="{{ route('enseignant.logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
                <span class="material-symbols-outlined">logout</span>
                <span class="font-label-md text-label-md">Déconnexion</span>
            </a>
            
            <form id="logout-form" action="{{ route('enseignant.logout') }}" method="POST" class="hidden">
                @csrf
            </form>
        </nav>
    </aside>
            ...
            <div class="flex items-center gap-1.5">
                ...
                <div class="relative">
                    <button id="profile-menu-button" type="button" class="flex items-center gap-1.5" aria-label="Menu du profil" aria-expanded="false">
                        <img alt="Photo de profil" class="w-8 h-8 rounded-full border border-outline-variant object-cover" src="{{ $headerUser?->image ? asset('storage/'.$headerUser->image) : 'https://ui-avatars.com/api/?background=1f108e&color=fff&name='.urlencode($headerUser?->name ?? 'Enseignant') }}">
                        <span class="text-sm font-medium text-on-surface hidden sm:inline-block">{{ $headerUser?->name ?? 'Enseignant' }}</span>
                    </button>
                    <div id="profile-menu" class="hidden absolute right-0 top-11 z-50 w-44 rounded-xl border border-outline-variant bg-surface-container-lowest p-1 shadow-xl">
                        <a class="block rounded-lg px-3 py-2 text-sm text-on-surface hover:bg-surface-container" href="{{ route('enseignant.parametres.index') }}">Mon profil</a>
                        <a class="block rounded-lg px-3 py-2 text-sm text-on-surface hover:bg-surface-container" href="{{ route('enseignant.parametres.index') }}">Paramètres</a>
