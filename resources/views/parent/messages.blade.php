...
<div class="mb-6 flex items-center justify-between gap-3 flex-wrap">
    ...
    <a class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-on-primary text-sm font-medium hover:opacity-90" href="{{ route('parent.messages.create') }}">
                ...
                @forelse($messages as $message)
                    ...
                    <tr class="border-b border-outline-variant/50 hover:bg-surface-container-low transition-colors {{ $isUnread ? 'bg-primary-fixed/10' : '' }}">
                        ...
                        <td class="px-6 py-4 text-right">
                            @if($isUnread)
                                <form method="POST" action="{{ route('parent.messages.read', $message->id) }}">
                                    @csrf
