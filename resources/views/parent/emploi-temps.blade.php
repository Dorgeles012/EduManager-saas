...
@section('title', 'Emploi du temps de '.$eleve->nom.' '.$eleve->prenom)
...
<div class="mb-6 flex items-center justify-between gap-3 flex-wrap">
    <div>
        ...
        <p class="text-sm text-on-surface-variant">
            Emploi du temps de {{ $eleve->nom }} {{ $eleve->prenom }}
            @if($year) · {{ $year->libelle }} @endif
            · {{ $eleve->classe?->nom ?? '—' }}
        </p>
    </div>
    <div class="flex gap-2">
        <a href="{{ route('parent.enfant.emploi-temps.print', $eleve) }}" target="_blank" class="inline-flex items-center gap-2 px-3 py-2 text-sm text-secondary hover:bg-secondary-container/20 rounded-lg">
...
<div class="bg-white rounded-xl border border-outline-variant custom-shadow p-6 overflow-x-auto">
    @if(! $hasEntries)
        <div class="text-center py-12">
            ...
            <p class="text-sm text-text-muted">L'emploi du temps de votre enfant n'a pas encore été publié.</p>
        </div>
    @else
