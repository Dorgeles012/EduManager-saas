    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <div class="flex items-center gap-3 mb-2">
                <a href="{{ route('client.series.index') }}" class="inline-flex items-center gap-2 text-sm text-primary hover:text-primary/80 transition-colors group">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-5">
            <div class="bg-white rounded-2xl shadow-sm border border-outline-variant/60 p-8 sticky top-6">
                ...
                <form action="{{ route('client.series.disciplines.store', $serie) }}" method="POST" class="space-y-5">
                    @csrf
                    
                    <!-- Matière -->
                    <div>
                        <label for="matiere_id" class="block text-sm font-medium text-on-surface mb-2">
                            Matière
                            <span class="text-alert-red">*</span>
                        </label>
                        <div class="relative">
                            ...
                            <select id="matiere_id" name="matiere_id" required 
                                    class="w-full pl-12 pr-4 py-3.5 rounded-xl border border-outline-variant bg-white focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition-all appearance-none text-[15px]">
                    ...
                    <!-- Coefficient -->
                    <div>
                        <label for="coefficient" class="block text-sm font-medium text-on-surface mb-2">
                            Coefficient
                            <span class="text-alert-red">*</span>
                        </label>
                        <div class="relative">
                            ...
                            <input id="coefficient" name="coefficient" type="number" min="1" max="100" step="1" required
                                   ...
                                   class="w-full pl-12 pr-4 py-3.5 rounded-xl border border-outline-variant bg-white focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition-all text-[15px]">
                        </div>
                        <p class="text-[12px] text-text-muted mt-1.5">Valeur comprise entre 1 et 100</p>
                    </div>

                    <!-- Bouton - Réduit -->
                    <button type="submit" 
                            class="w-full py-3 rounded-xl bg-gradient-to-r from-primary to-primary-container text-white font-medium hover:shadow-lg hover:opacity-90 transition-all flex items-center justify-center gap-2 text-[14px]">
        ...
        <div class="lg:col-span-7">
            <div class="bg-white rounded-2xl shadow-sm border border-outline-variant/60 overflow-hidden">
                ...
                <div class="px-5 py-3.5 border-b border-outline-variant/60 bg-surface-container-low/30 flex items-center justify-between">
                    ...
                    <span class="px-2.5 py-0.5 bg-primary/10 text-primary rounded-full text-xs font-medium">
                        {{ $disciplines->count() }}
                    </span>
                </div>
                ...
                <div class="overflow-x-auto">
                        ...
                        <table class="w-full text-left">
                            <thead>
                                <tr class="bg-surface-container-low/30 border-b border-outline-variant/30">
                                    <th class="px-5 py-2.5 text-[11px] font-semibold text-text-muted uppercase tracking-wider">
                                    ...
                                    <th class="px-5 py-2.5 text-[11px] font-semibold text-text-muted uppercase tracking-wider text-right">
                                        Actions
                                    </th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-outline-variant/20">
                                @foreach($disciplines as $discipline)
                                    <tr class="hover:bg-surface-container-low/30 transition-colors">
                                        ...
                                        <td class="px-5 py-3">
                                            <form action="{{ route('client.series.disciplines.update', [$serie, $discipline]) }}" method="POST" class="flex items-center gap-2">
                                                @csrf 
                                                @method('PUT')
                                                <div class="relative">
                                                    <input name="coefficient" type="number" min="1" max="100" step="1" 
                                                           value="{{ $discipline->pivot->coefficient }}" required
                                                           class="w-16 px-2 py-1 rounded-lg border border-outline-variant bg-white focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition-all text-center text-[13px]">
                                                </div>
                                                <!-- Bouton Enregistrer toujours visible -->
                                                <button type="submit" 
                                                        class="p-1.5 rounded-lg text-primary hover:bg-primary/10 transition-all text-sm font-medium"
                                                        title="Mettre à jour">
                                        ...
                                        <td class="px-5 py-3 text-right">
                                            <form action="{{ route('client.series.disciplines.destroy', [$serie, $discipline]) }}" method="POST" class="delete-discipline-form inline">
                                                ...
                                                @method('DELETE')
                                                <button type="submit" 
                                                        class="p-1.5 rounded-lg text-text-muted hover:text-alert-red hover:bg-alert-red/10 transition-all"
                    ...
                    @else
                        ...
                        <div class="flex flex-col items-center justify-center py-12 px-4">
                            ...
                            <p class="text-[13px] text-text-muted text-center max-w-sm">
                                Aucune discipline n'est associée à cette série.
                            </p>
                        </div>
                    @endif
                </div>
                ...
                <!-- Pied de tableau -->
                @if($disciplines->isNotEmpty())
                    <div class="px-5 py-2.5 border-t border-outline-variant/60 bg-surface-container-low/30 flex items-center justify-between">
                        <span class="text-[11px] text-text-muted">
                            Total : <span class="font-medium text-on-surface">{{ $disciplines->count() }}</span>
                        </span>
                        <span class="text-[11px] text-text-muted">
                            Coeff. : <span class="font-medium text-on-surface">{{ $disciplines->sum('pivot.coefficient') }}</span>
                        </span>
                    </div>
                @endif
            </div>
...
@push('scripts')
    ...
    <script>
        // Auto-remplir le coefficient en fonction de la matière sélectionnée
        ...
        @if(session('success'))
            Swal.fire({
                ...
                customClass: {
                    popup: 'rounded-2xl',
                    confirmButton: 'rounded-lg px-4 py-2 bg-primary',
                }
            });
        @endif
        ...
        @if($errors->any())
            Swal.fire({
                title: 'Erreur',
                html: @json(collect($errors->all())->map(fn($message) => "<p class='text-left text-sm'>• ".$message."</p>")->join('')),
                icon: 'error',
                confirmButtonText: 'OK',
                customClass: {
                    popup: 'rounded-2xl',
