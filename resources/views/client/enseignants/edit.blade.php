...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Modifier un enseignant</h2>
...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
    <div class="px-5 py-4 border-b border-gray-100 flex items-center gap-2.5">
        ...
        <div>
            <h3 class="text-sm font-semibold text-gray-900">Modifier l'enseignant</h3>
                ...
                <div class="bg-gray-50/50 rounded-2xl border border-gray-100 p-4 space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                            ...
                            <input name="nom" type="text" value="{{ old('nom', $enseignant->nom) }}" required placeholder="Entrez le nom"
                        ...
                        <div>
                            ...
                            <input name="prenoms" type="text" value="{{ old('prenoms', $enseignant->prenoms) }}" required placeholder="Entrez les prénoms"
                                   class="w-full bg-white border border-gray-200 rounded-lg text-xs py-2.5 px-3 focus:border-amber-500 focus:ring-2 focus:ring-amber-100 outline-none transition">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                            ...
                            <input name="matricule" type="text" value="{{ old('matricule', $enseignant->matricule) }}" required placeholder="ENS-2024-001"
                                   class="w-full bg-white border border-gray-200 rounded-lg text-xs py-2.5 px-3 focus:border-amber-500 focus:ring-2 focus:ring-amber-100 outline-none transition">
                        ...
                        <div>
                            ...
                            <input name="nombre_annees_enseignement" type="number" min="0" max="80"
                                   value="{{ old('nombre_annees_enseignement', $enseignant->nombre_annees_enseignement) }}" required placeholder="0"
                                   class="w-full bg-white border border-gray-200 rounded-lg text-xs py-2.5 px-3 focus:border-amber-500 focus:ring-2 focus:ring-amber-100 outline-none transition">
                    ...
                    <div>
                        ...
                        <input name="email" type="email" value="{{ old('email', $enseignant->email) }}" required placeholder="exemple@mail.com"
                               class="w-full bg-white border border-gray-200 rounded-lg text-xs py-2.5 px-3 focus:border-amber-500 focus:ring-2 focus:ring-amber-100 outline-none transition">
                    ...
                    <div>
                        ...
                        <input name="telephone" type="tel" value="{{ old('telephone', $enseignant->telephone) }}" required placeholder="01 02 03 04 05"
                               class="w-full bg-white border border-gray-200 rounded-lg text-xs py-2.5 px-3 focus:border-amber-500 focus:ring-2 focus:ring-amber-100 outline-none transition">
                    ...
                    <div>
                        ...
                        <div class="flex gap-2">
                            <label class="flex-1 cursor-pointer">
                                <input type="radio" name="sexe" value="Masculin" {{ old('sexe', $enseignant->sexe) === 'Masculin' ? 'checked' : '' }} required class="peer sr-only">
                            ...
                            <label class="flex-1 cursor-pointer">
                                <input type="radio" name="sexe" value="Féminin" {{ old('sexe', $enseignant->sexe) === 'Féminin' ? 'checked' : '' }} class="peer sr-only">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                <label class="cursor-pointer inline-flex items-center gap-1.5 px-3 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-600 text-xs font-semibold rounded-lg transition">
                                ...
                                @if($enseignant->photo)
                                    <p class="text-[10px] text-amber-600 mt-0.5">Photo actuelle conservée si inchangée</p>
                                @endif
                            </div>
                        </div>
                ...
                <div class="bg-gray-50/50 rounded-2xl border border-gray-100 p-4 space-y-4">
                    ...
                    <div>
                        ...
                        <div class="relative" id="matiereContainer">
                            <div class="flex flex-wrap gap-1.5 p-2 border border-gray-200 rounded-lg focus-within:ring-2 focus-within:ring-amber-100 focus-within:border-amber-500 transition bg-white min-h-[42px]" id="matiereTags">
                            ...
                            <div id="matiereSuggestions" class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto hidden">
                                @foreach($matieres as $m)
                                    <div class="suggestion-item px-3 py-2 hover:bg-amber-50 cursor-pointer text-xs flex items-center justify-between" data-id="{{ $m->id }}" data-name="{{ $m->nom }}">
                                        <span>{{ $m->nom }}</span>
                                        <span class="text-xs text-gray-400">+</span>
                                    </div>
                                @endforeach
                            </div>
                            <input type="hidden" id="matiereIds" value="{{ implode(',', old('matiere_ids', $enseignant->matieres->pluck('id')->all())) }}">
                        </div>
                    ...
                    <div>
                        ...
                        <div class="relative" id="classeContainer">
                            <div class="flex flex-wrap gap-1.5 p-2 border border-gray-200 rounded-lg focus-within:ring-2 focus-within:ring-amber-100 focus-within:border-amber-500 transition bg-white min-h-[42px]" id="classeTags">
                            ...
                            <div id="classeSuggestions" class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto hidden">
                                @foreach($classes as $c)
                                    <div class="suggestion-item px-3 py-2 hover:bg-amber-50 cursor-pointer text-xs flex items-center justify-between" data-id="{{ $c->id }}" data-name="{{ $c->nom }}">
                                        <span>{{ $c->nom }}</span>
                                        <span class="text-xs text-gray-400">+</span>
                                    </div>
                                @endforeach
                            </div>
                            <input type="hidden" id="classeIds" value="{{ implode(',', old('classe_ids', $enseignant->classes->pluck('id')->all())) }}">
                        </div>
                    ...
                    <div>
                        ...
                        <div class="relative" id="serieContainer">
                            <div class="flex flex-wrap gap-1.5 p-2 border border-gray-200 rounded-lg focus-within:ring-2 focus-within:ring-amber-100 focus-within:border-amber-500 transition bg-white min-h-[42px]" id="serieTags">
                            ...
                            <div id="serieSuggestions" class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto hidden">
                                @foreach($series as $s)
                                    <div class="suggestion-item px-3 py-2 hover:bg-amber-50 cursor-pointer text-xs flex items-center justify-between" data-id="{{ $s->id }}" data-name="{{ $s->nom_serie ?? $s->nom }}">
                                        <span>{{ $s->nom_serie ?? $s->nom }}</span>
                                        <span class="text-xs text-gray-400">+</span>
                                    </div>
                                @endforeach
                            </div>
                            <input type="hidden" id="serieIds" value="{{ implode(',', old('serie_ids', $enseignant->series->pluck('id')->all())) }}">
                        </div>
...
<style>
    ...
    .tag-item .remove-tag:hover { background: #fcd34d; color: #78350f; }
    @keyframes tagFadeIn {
        from { opacity: 0; transform: scale(0.95); }
        to { opacity: 1; transform: scale(1); }
    }
    .suggestion-item:hover .text-gray-400 { color: #d97706 !important; }
    ...
    function initAutocomplete(config) {
        ...
        searchInput.addEventListener('input', function() {
            ...
            suggestions.innerHTML = filtered.length === 0
                ...
                : filtered.map(item => `<div class="suggestion-item px-3 py-2 hover:bg-amber-50 cursor-pointer text-xs flex items-center justify-between" data-id="${item.id}" data-name="${item.name}"><span>${item.name}</span><span class="text-xs text-gray-400">+</span></div>`).join('');
