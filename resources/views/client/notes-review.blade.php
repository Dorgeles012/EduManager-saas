...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-3 mb-5">
    <div>
        <div class="flex items-center gap-2 mb-1">
            <a href="{{ route('client.notes.index') }}" class="text-xs text-primary hover:underline flex items-center gap-1">
                <span class="material-symbols-outlined text-sm">arrow_back</span>
                Retour aux validations
            </a>
        </div>
        <h2 class="font-headline-md text-2xl text-primary mb-0.5">Validation Finale & Publication — {{ $classe->nom }}</h2>
    ...
    <div class="flex items-center gap-2">
        <button type="button" class="px-3 py-1.5 bg-alert-red/10 text-alert-red hover:bg-alert-red hover:text-white rounded-lg text-xs font-bold transition-all flex items-center gap-1" onclick="openRejectModal()">
        ...
        <form method="POST" action="{{ route('client.notes.publier') }}" class="inline">
            ...
            <button type="submit" class="bg-primary text-white text-xs px-4 py-1.5 rounded-lg font-bold hover:opacity-90 flex items-center gap-1.5 shadow-sm" onclick="return confirm('Valider et publier ces notes ? Les bulletins scolaires seront mis à jour avec les moyennes officielles.');">
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
    ...
    <div class="bg-surface-container-lowest p-4 rounded-xl border border-outline-variant shadow-ambient">
        <span class="text-[10px] text-text-muted uppercase font-bold tracking-wider">Total des évaluations</span>
...
<div class="fixed inset-0 z-[100] hidden items-center justify-center p-4" id="rejectModal">
    ...
    <div class="bg-surface-container-lowest w-full max-w-md rounded-xl shadow-2xl border border-outline-variant overflow-hidden transform transition-all duration-300 relative z-10 p-6">
        ...
        <p class="text-xs text-text-muted mb-4">
            Indiquez le motif du rejet. L'enseignant sera notifié et devra corriger les notes.
        </p>
        ...
        <form method="POST" action="{{ route('client.notes.rejeter') }}">
            ...
            <div class="mb-4">
                ...
                <textarea name="rejet_motif" rows="3" class="w-full bg-surface rounded-lg border-outline-variant text-xs p-2.5 focus:border-alert-red focus:ring-alert-red" placeholder="Ex: Incohérence constatée..." required></textarea>
            </div>
