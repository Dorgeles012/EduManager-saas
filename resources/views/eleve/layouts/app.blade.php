...
<head>
    ...
    <title>@yield('title', 'EduManager - Espace Élève')</title>
...
<body class="font-body-md text-body-md overflow-x-hidden">
    ...
    <aside class="fixed left-0 top-0 h-screen w-[260px] bg-surface-container-lowest flex flex-col border-r border-outline-variant z-50">
        ...
        <nav class="sidebar-nav mt-2">
            ...
            <a class="flex items-center gap-3 px-6 py-3 text-on-surface-variant hover:bg-surface-container transition-colors {{ $currentRoute === 'eleve.dashboard' ? 'nav-active' : '' }}" href="{{ route('eleve.dashboard') }}">
                <span class="material-symbols-outlined">dashboard</span>
                <span class="font-label-md text-label-md">Tableau de bord</span>
            </a>

            <a class="flex items-center gap-3 px-6 py-3 text-on-surface-variant hover:bg-surface-container transition-colors {{ $currentRoute === 'eleve.scolarite' ? 'nav-active' : '' }}" href="{{ route('eleve.scolarite') }}">
                <span class="material-symbols-outlined">school</span>
                <span class="font-label-md text-label-md">Ma scolarité</span>
            </a>

            <a class="flex items-center gap-3 px-6 py-3 text-on-surface-variant hover:bg-surface-container transition-colors {{ $currentRoute === 'eleve.notes' ? 'nav-active' : '' }}" href="{{ route('eleve.notes') }}">
                <span class="material-symbols-outlined">fact_check</span>
                <span class="font-label-md text-label-md">Mes notes</span>
            </a>

            <a class="flex items-center gap-3 px-6 py-3 text-on-surface-variant hover:bg-surface-container transition-colors {{ $currentRoute === 'eleve.bulletins' ? 'nav-active' : '' }}" href="{{ route('eleve.bulletins') }}">
                <span class="material-symbols-outlined">description</span>
                <span class="font-label-md text-label-md">Mes bulletins</span>
            </a>

            <a class="flex items-center gap-3 px-6 py-3 text-on-surface-variant hover:bg-surface-container transition-colors {{ $currentRoute === 'eleve.classe' ? 'nav-active' : '' }}" href="{{ route('eleve.classe') }}">
                <span class="material-symbols-outlined">group</span>
                <span class="font-label-md text-label-md">Ma classe</span>
            </a>

<a class="flex items-center gap-3 px-6 py-3 text-on-surface-variant hover:bg-surface-container transition-colors {{ str_starts_with($currentRoute, 'eleve.emploi-temps') ? 'nav-active' : '' }}" href="{{ route('eleve.emploi-temps') }}">
                <span class="material-symbols-outlined">calendar_month</span>
                <span class="font-label-md text-label-md">Mon emploi du temps</span>
            </a>

            <a class="flex items-center gap-3 px-6 py-3 text-on-surface-variant hover:bg-surface-container transition-colors {{ $currentRoute === 'eleve.notifications' ? 'nav-active' : '' }}" href="{{ route('eleve.notifications') }}">
            ...
            <a class="flex items-center gap-3 px-6 py-3 text-on-surface-variant hover:bg-surface-container transition-colors {{ $currentRoute === 'eleve.profil' ? 'nav-active' : '' }}" href="{{ route('eleve.profil') }}">
            ...
            <form id="logout-form" action="{{ route('eleve.logout') }}" method="POST" class="hidden">
                @csrf
            </form>
        </nav>
    </aside>
            ...
            <div class="flex items-center gap-1.5 ml-auto">
                ...
                <div class="relative">
                    <button id="profile-menu-button" type="button" class="flex items-center gap-1.5" aria-label="Menu du profil">
                        <img alt="Photo de profil" class="w-8 h-8 rounded-full border border-outline-variant object-cover" src="{{ optional(auth()->user()->eleve)->photo ? asset('storage/'.optional(auth()->user()->eleve)->photo) : 'https://ui-avatars.com/api/?background=1f108e&color=fff&name='.urlencode(optional(auth()->user()->eleve)->nom ?? 'Élève') }}">
                        <span class="text-sm font-medium text-on-surface hidden sm:inline-block">{{ optional(auth()->user()->eleve)->nom ?? 'Élève' }}</span>
                    </button>
