...
<div class="mb-6 flex items-center justify-between gap-3 flex-wrap">
    <div>
        ...
        <p class="text-sm text-on-surface-variant">Retrouvez toutes les notifications qui vous concernent</p>
    </div>
    @if($unreadCount > 0)
        <form method="POST" action="{{ route('eleve.notifications.read-all') }}">
            @csrf
            ...
            <div class="p-5 flex items-start gap-4 {{ $isUnread ? 'bg-primary-fixed/10' : '' }}">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <div class="flex items-center gap-3 mt-3">
                        @if($isUnread)
                            <form method="POST" action="{{ route('eleve.notifications.read', $recipient) }}">
                                @csrf
                        ...
                        @endif
                        <form method="POST" action="{{ route('eleve.notifications.destroy', $recipient) }}" onsubmit="return confirm('Supprimer cette notification ?')">
                            @csrf
                            @method('DELETE')
