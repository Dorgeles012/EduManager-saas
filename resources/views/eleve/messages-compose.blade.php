...
<div class="mb-6 flex items-center justify-between gap-3 flex-wrap">
    <div>
        ...
        <p class="text-sm text-on-surface-variant">Envoyer un message</p>
    </div>
    <a class="inline-flex items-center gap-2 px-3 py-2 text-sm text-gray-600 hover:text-primary rounded-lg" href="{{ route('eleve.messages') }}">
...
<div class="max-w-2xl bg-surface-container-lowest rounded-xl custom-shadow border border-outline-variant p-6">
    <form method="POST" action="{{ route('eleve.messages.store') }}" class="space-y-5">
        @csrf
